...
int main(int argc, char **argv) {
    ...
    double util, scaled_util, throughput, avg_mm_util, avg_mm_throughput, perfindex; //, secs, ops, p1, p2;
    int numcorrect;
    ...
    if (errors == 0) {
        ...
        // avg_mm_throughput = ops / 1000 / secs;
        ...
        printf("Score = (%.3f [scaled util])^2 * (%.3f [avg Kops]) / 100 = %.0f/100\n",
               scaled_util,
               avg_mm_throughput,
               //p1*100,
               //p2*100,
        ...
        //double adjusted_kops = avg_mm_util * avg_mm_util * avg_mm_util * avg_mm_throughput / 1000.0;
        //printf("Adjusted KOps:  Util^3 * KOps: %8.1f\n", adjusted_kops);
